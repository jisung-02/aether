...
pub fn new_window_test() {
  ...
  flow_control.window_available(window)
...
pub fn handle_connection_window_update_test() {
  ...
  let consumed =
    flow_control.consume_send_window(controller, 10_000)
    |> should.be_ok()
...
pub fn update_initial_window_size_overflow_test() {
  ...
  let result =
    flow_control.update_initial_window_size(controller, max_window_size + 1)
  result |> should.be_error()
}
  ...
  case result {
    Ok(#(_, pending)) -> {
      case pending {
        ...
        // Expected
        Some(_) -> panic
        // Should not have pending update
      }
    }
    Error(_) -> panic
  }
