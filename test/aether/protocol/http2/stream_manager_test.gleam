...
pub fn new_client_manager_test() {
  ...
  manager.next_stream_id |> should.equal(1)  // Client starts with odd IDs
...
pub fn new_server_manager_test() {
  ...
  manager.next_stream_id |> should.equal(2)  // Server starts with even IDs
  manager.active_stream_count |> should.equal(0)
}
...
pub fn create_stream_client_test() {
  ...
  case result {
    Ok(#(updated_manager, stream)) -> {
      stream.id |> should.equal(1)  // First client stream is 1
      stream.state |> should.equal(Open)  // Stream is opened
      updated_manager.next_stream_id |> should.equal(3)  // Next will be 3
...
pub fn create_stream_server_test() {
  ...
  case result {
    Ok(#(updated_manager, stream)) -> {
      stream.id |> should.equal(2)  // First server stream is 2
      updated_manager.next_stream_id |> should.equal(4)  // Next will be 4
    }
    Error(_) -> panic
  }
}
...
pub fn create_multiple_streams_test() {
  ...
  // Create first stream
  let #(manager1, stream1) = stream_manager.create_stream(manager) 
    |> should.be_ok()
  stream1.id |> should.equal(1)
  
  // Create second stream
  let #(manager2, stream2) = stream_manager.create_stream(manager1) 
    |> should.be_ok()
  stream2.id |> should.equal(3)
  
  // Create third stream
  let #(manager3, stream3) = stream_manager.create_stream(manager2) 
    |> should.be_ok()
...
pub fn get_or_create_peer_stream_server_test() {
  ...
  case result {
    Ok(#(updated_manager, stream)) -> {
      ...
      stream.state |> should.equal(Idle)  // New peer stream starts Idle
...
pub fn get_existing_peer_stream_test() {
  ...
  // Create stream
  let #(m1, s1) = stream_manager.get_or_create_peer_stream(manager, 1) 
    |> should.be_ok()
  
  // Get same stream again
  let #(m2, s2) = stream_manager.get_or_create_peer_stream(m1, 1)
    |> should.be_ok()
...
pub fn stream_close_decrements_active_count_test() {
  ...
  let m2 = stream_manager.apply_stream_event(m1, 1, SendRstStream)
    |> should.be_ok()
...
pub fn handle_rst_stream_test() {
  let manager = stream_manager.new_server()
  let #(m1, _) = stream_manager.get_or_create_peer_stream(manager, 1)
    |> should.be_ok()
...
pub fn update_initial_window_size_overflow_test() {
  ...
  let result = stream_manager.update_initial_window_size(m1, 2_147_483_647 + 100)
  result |> should.be_error()
}
...
pub fn cleanup_closed_streams_test() {
  ...
  stream_manager.total_count(m3) |> should.equal(2)  // Both streams still in map
  
  // Cleanup, keep 0 closed streams
  let m4 = stream_manager.cleanup_closed_streams(m3, 0)
  
  stream_manager.total_count(m4) |> should.equal(1)  // Only active stream remains
  stream_manager.has_stream(m4, 1) |> should.equal(False)  // Closed stream removed
  stream_manager.has_stream(m4, 3) |> should.equal(True)   // Active stream remains
}
