...
pub fn handle_window_update_connection_level_test() {
  ...
  let window_update = WindowUpdateF(
    FrameHeader(length: 4, frame_type: WindowUpdate, flags: 0, stream_id: 0),
    WindowUpdateFrame(window_size_increment: 32768),
  )
...
pub fn handle_window_update_stream_level_test() {
  ...
  let window_update = WindowUpdateF(
    ...
    WindowUpdateFrame(window_size_increment: 16384),
  )
...
pub fn default_settings_values_test() {
  ...
  settings.initial_window_size |> should.equal(65535)
  settings.max_frame_size |> should.equal(16384)
  settings.max_header_list_size |> should.equal(16384)
}
...
pub fn build_response_no_body_test() {
  ...
  let #(_new_conn, frames) = connection.build_response(
    conn,
    1,  // stream_id
    200,  // status
