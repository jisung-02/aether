...
pub fn process_frame(
  ...
  case connection.handle_frame(state.connection, frame) {
    ...
    connection.RequestComplete(conn, stream_id, headers, body) -> {
      ...
      let #(final_conn, response_frames) = handlers.build_response_frames(
        conn,
        stream_id,
        response,
      )
...
pub fn demo() {
  ...
  let request1 = handlers.new_request(
    1,  // stream_id
    [
      #(":method", "GET"),
      #(":path", "/api/users"),
  ...
  let #(response1, store1) = handlers.handle_request(request1, initial_state.store)
  ...
  let request2 = handlers.new_request(
    3,  // stream_id (odd for client-initiated)
    [
      #(":method", "GET"),
      #(":path", "/api/users/1"),
      #(":scheme", "https"),
  ...
  let request3 = handlers.new_request(
    5,
    [
      #(":method", "POST"),
      #(":path", "/api/users"),
  ...
  let request4 = handlers.new_request(
    7,
    [
      #(":method", "DELETE"),
      #(":path", "/api/users/2"),
  ...
  let request5 = handlers.new_request(
    9,
    [
      #(":method", "GET"),
      #(":path", "/api/users/999"),
